...
{ kana: "ライバル心に火が付いた。いざ尋常に勝負だ、お前ら！！", romaji: ["raibarusinnnihigatuita.izazinnzyounisyoubuda,omaera!!" , "raibarushinnihigatuita.izazinnzyounishoubuda,omaera!!"] }
  ...
  sushiList: [
    ...
    { kana: "言葉の残響", romaji: ["kotoba", "no", "zankyou"] },
    { kana: "ɥɐdɐɐɯ", romaji: ["yapaamu"] }, // 擬似反転文字
    { kana: "風は沈黙した", romaji: ["kaze", "wa", "chinmokushita"] },
    { kana: "記憶の花が咲く", romaji: ["kioku", "no", "hana", "ga", "saku"] },
    { kana: "kon kon kon", romaji: ["kon", "kon", "kon"] },
    { kana: "Verweile doch", romaji: ["verweile", "doch"] }, // 多言語要素
    { kana: "光をなぞる夢", romaji: ["hikari", "wo", "nazoru", "yume"] },
    { kana: "時は打鍵で綴られる", romaji: ["toki", "wa", "dakende", "tsuzurareru"] },
    { kana: "煙の中で言葉が踊る", romaji: ["kemuri", "no", "nakade", "kotoba", "ga", "odoru"] },
    { kana: "qulia ese-no thalti", romaji: ["qulia", "eseno", "thalti"] }, // 擬似言語
    { kana: "鼓動の滲み", romaji: ["kodou", "no", "nijimi"] },
    { kana: "sennmitsu", romaji: ["sennmitsu"] }, // 意味不明語
    { kana: "視えぬ風を打て", romaji: ["mienu", "kaze", "wo", "ute"] }
   ]
 }
};
...
// 初期状態
let { timeLimit, sushiTimeLimit, sushiList } = config[difficulty];
let currentSushi = {};
let previousIndex = -1;
let typed = "";
let score = 0;
...
function endGame() {
  ...
  // クリア判定＋コメント表示
  let clearLine = { easy: 10, normal: 15, hard: 20, lunatic: 23, SuperLunatic: 26 };
...
function startCountdownAndGame() {
  ...
  const countdownInterval = setInterval(() => {
    ...
    if (countdown > 0) {
      ...
      countdownElement.textContent = ""; // カウント表示を消す
      startGame(); // ←ここで制限時間カウント開始！
    }
  }, 1000);
}
