...
function getComment(score) {
  if (score === 0) return "さあ、もてなしましょう！";
  else if (score <= 10) return "悪くないスタートです！";
  else if (score <= 20) return "見習いレベルです🍃";
  else if (score <= 40) return "お茶職人の腕前ですね☕";
...
function getTitle(difficulty) {
  switch (difficulty) {
    case "easy": return "見習い茶道人🍵";
    case "normal": return "一人前の茶道人🍁";
    case "hard": return "輝く茶道人🔥";
    case "lunatic": return "茶道の神👑";
    case "SuperLunatic": return "人間国宝級の茶道人💎";
...
function handleTyping(event) {
  ...
  if (candidates.some(r => r.startsWith(typed))) {
    ...
    if (candidates.includes(typed)) {
      score++;
      scoreElement.textContent = `師匠からの評価: ${score}点`;
...
function endGame() {
  ...
  if (score >= clearLine[difficulty]) {
    commentElement.textContent = `🎉 おもてなしできました！称号: ${getTitle(difficulty)}`;
  } else {
    commentElement.textContent = "おもてなし不足…またの挑戦を！🥢";
  }
...
function startCountdownAndGame() {
  ...
  countdownElement.textContent = `おもてなし開始まで… ${countdown}`;
...
function startTimer() {
  gameTimer = setInterval(() => {
    timeLimit--;
    timerElement.textContent = `おもてなしできる時間: あと${timeLimit}秒`;
    if (timeLimit <= 0) {
...
function startGame() {
  showNewSushi();
  scoreElement.textContent = `師匠からの評価: ${score}`;
  commentElement.textContent = getComment(score);
  timerElement.textContent = `おもてなしできる時間: あと${timeLimit}秒`;
