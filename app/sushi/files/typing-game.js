...
const config = {
  ...
  normal: {
    ...
    sushiList: [
      ...
      { kana: "カツオシーチキン", romaji: ["katuosi-tikinn" , "katsuoshi-chikin"] },
      { kana: "マグロシーチキン", romaji: ["magurosi-tikinn" , "maguroshi-chikinn"] },
      { kana: "カツオのたたき(!?)", romaji: ["katuonotataki" , "katsuonotataki"] },
      { kana: "カツオ節", romaji: ["katuobusi" , "katsuobushi"] },
      { kana: "シーチキンL", romaji: ["si-tikinnl" , "shi-chikinnl"] }
    ]
  },
  hard: {
    timeLimit: 120,
  ...
  lunatic: {
    ...
    sushiList: [
      ...
      { kana: "早口ことばに勝てる寿司職人は今や伝説の存在らしい", romaji: ["hayakutikotobanikaterusushisyokuninhamayadensetsunosonzairashii"] },
      { kana: "シャリの温度で議論が起きるのは一流寿司屋だけだと思っていた", romaji: ["sharinonondodegironnagaokirunohairyuusushiyadakedatoomotteita" , "syarinoonndodegironngaokirunohaitiryuususiyadakedatoomotteita"] },
      { kana: "打ちすぎて指が寿司のように柔らかくなった気がしてきた", romaji: ["uchisugiteyubigasushinoyouniyawarakakunattakigashitekita" , "utisugiteyubigasusinoyouniyawarakakunattakigasitekita"] },
      { kana: "酢飯に人生を見いだすのはもはや悟りの領域としか言いようがない", romaji: ["sumesinijinseiwoiidasunohamohayasatorinoryouikitoshakaiyouganai"] },
      { kana: "寿司を打ち続けることで世界平和が訪れると本気で信じている人がいる", romaji: ["sushiwoutituzukerukotodesekaiheiwagaotozurerutohonkideshinjiteiruhitogairu" , "susiwoutituzukerukotodesekaiheiwagaotozurerutohonnkidesinnziteiruhitogairu"] },
  ...
  SuperLunatic: {
    ...
    sushiList: [
      ...
      { kana: "きつつき", romaji: ["kitutuki" , "kitsutsuki"] },
      ...
      { kana: "騒がしい... うせな！煙草(たばこ)の味が濁る", romaji: ["sawagasii...usena!tabakonoaziganigoru"] },
      { kana: "人でないものは何というか知ってるか? 私は知らねえよ", romaji: ["hitodenaimonohananntoiukasitteruka?watasihasiraneeyo"] },
      { kana: "くぁwせdrftgyふじこlp (うわああああ)", romaji: ["uwaaaaa"] },
...
// --- 互換性維持：元のplay.htmlから呼ばれる関数群 ---

function detectOS() { return "Unknown OS"; }
function detectBrowserInfo() { return "Unknown Browser"; }
...
function getTitle(difficulty) {
  const titles = { easy: "見習い🍵", normal: "一人前🐟", hard: "輝く🔥", lunatic: "神👑", SuperLunatic: "頂点" };
  return titles[difficulty] || "未知の称号";
}

// ポップアップ通知
function showPointBonusNotice(p) {
  const n = document.createElement("div");
  n.style = "position:fixed;top:35%;left:50%;transform:translate(-50%,-50%);font-size:2.5em;color:#ff4500;font-weight:bold;z-index:1000;background:rgba(255,255,255,0.9);padding:15px 30px;border:3px solid #ff4500;border-radius:15px;pointer-events:none;animation:fadeOut 1s forwards;";
  n.textContent = `${p}点ネタです！`;
  document.body.appendChild(n);
  setTimeout(() => n.remove(), 1000);
}
const style = document.createElement("style");
style.innerText = "@keyframes fadeOut { from { opacity: 1; } to { opacity: 0; } }";
...
function showNewSushi() {
  ...
  // ポイント・フォント自動判定
  let p = 1;
  if (currentSushi.kana.length >= 15) {
    p = 3;
    sushiElement.style.fontFamily = '"Yu Mincho", serif';
    showPointBonusNotice(3);
  } else if (currentSushi.kana.length >= 10) {
    p = 2;
    sushiElement.style.fontFamily = 'sans-serif';
    showPointBonusNotice(2);
  } else {
    sushiElement.style.fontFamily = 'sans-serif';
  }
  currentSushi.tempPoints = p;
...
function handleTyping(event) {
  ...
  if (candidates.some(r => r.startsWith(typed))) {
    ...
    if (candidates.includes(typed)) {
      score += (currentSushi.tempPoints || 1);
...
function endGame() {
  ...
  const clearLine = { easy: 10, normal: 15, hard: 20, lunatic: 23, SuperLunatic: 26 };
  commentElement.textContent = score >= clearLine[difficulty] ? `🎉 クリア！称号: ${getTitle(difficulty)}` : "修行不足…またの挑戦を！🥢";
